...
Передайте сіль і отриманий хеш під час release-збірки:
...
Gradle генерує `BuildConfig.ADMIN_PIN_SALT` і `BuildConfig.ADMIN_PIN_HASH` у
namespace `com.example.montasch`. У `buildFeatures` мають залишатися ввімкненими
`buildConfig` і `resValues`.
